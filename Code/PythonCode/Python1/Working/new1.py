    ...
    while len(data) < max_entries:  # Stop after collecting 100 entries
        if ser.in_waiting > 0:
            ...
            if len(parts) == 3:
                ...
                secs = int(time_data.split('m')[1].replace('s', ''))
                ...
                data.append([time_data, float(temp_data), float(voltage_data), elapsed_time])

                # For testing, print the parsed data
                print(f"Time: {time_data}, Temp: {temp_data}, Voltage: {voltage_data}, Elapsed Time: {elapsed_time:.2f} seconds")
